...
def pick_without_back(position, pitch, yaw, gripper_angle, gripper_depth, joints_pub, kinematics_client, interpolation=False):
    ...
    if not stop:
        ...
        if interpolation:
            ...
            set_servo_position(joints_pub, 1.0, ((6, servo_data[-1][0]),))
            
            msg = set_pose_target(position, pitch, [-90.0, 90.0], 1.0, duration=1.5)
            res = send_request(kinematics_client, msg)
            servo_data = np.array(res.pulse).reshape(-1, 5).tolist()
            for i in servo_data:
                set_servo_position(joints_pub, dt, ((5, i[1]), (4, i[2]), (3, i[3])))
                time.sleep(dt - d)
        else:
            msg = set_pose_target(position, pitch, [-90.0, 90.0], 1.0)
            res = send_request(kinematics_client, msg)
            servo_data = res.pulse
            set_servo_position(joints_pub, 1.0, ((6, servo_data[0]),))
            time.sleep(1)

            set_servo_position(joints_pub, 1, ((5, servo_data[1]), (4, servo_data[2]), (3, servo_data[3])))
            time.sleep(1)

        set_servo_position(joints_pub, 0.5, ((2, yaw),))
        time.sleep(0.5)
        ...
        if not stop:
            ...
            if interpolation:
                ...
                res = send_request(kinematics_client, msg)
                servo_data = res.pulse
                set_servo_position(joints_pub, 0.5, ((5, servo_data[1]), (4, servo_data[2]), (3, servo_data[3])))
                time.sleep(0.5)
    ...
    if pick_without_back(position, pitch, yaw, gripper_angle, gripper_depth, joints_pub, kinematics_client, interpolation):
        if not stop:
            if interpolation:
                ...
                res = send_request(kinematics_client, msg)
                servo_data = np.array(res.pulse).reshape(-1, 5).tolist()
                set_servo_position(joints_pub, 0.5, ((2, 500),))
                time.sleep(0.02)
                for i in servo_data:
                    set_servo_position(joints_pub, dt, ((5, i[1]), (4, i[2]), (3, i[3])))
                    time.sleep(dt - d)
    ...
    if not stop:
        ...
        if not stop:
            ...
            if interpolation:
                ...
                set_servo_position(joints_pub, 0.5, ((5, servo_data[1]), (4, servo_data[2]), (3, servo_data[3])))
                time.sleep(0.5)
            set_servo_position(joints_pub, 0.5, ((1, 200),))
            return True
    return False
