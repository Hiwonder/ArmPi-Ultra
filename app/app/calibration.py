...
class CalibrationNode(Node):
    ...
    def __init__(self, name):
        ...
        # 【改动 2】: 新增逻辑，从 camera_info.yaml 加载手眼变换矩阵
        with open(self.camera_info_file, 'r') as f:
            camera_info_config = yaml.safe_load(f)
            # 从YAML加载矩阵并转换为numpy数组
            self.hand2cam_tf_matrix = np.array(camera_info_config['hand2cam_tf_matrix'])
        ...
        # 【改动 3】: 此处使用从YAML文件加载的 self.hand2cam_tf_matrix
    ...
    def enter_srv_callback(self,request, response):
        ...
        if self.camera_type == "usb_cam":
            ...
            self.camera_info_sub = self.create_subscription(CameraInfo, '/depth_cam/rgb/camera_info', self.camera_info_callback, 1)
        set_servo_position(self.joints_pub, 1.5, ((6, 500), (5, 600), (4, 825), (3, 110), (2, 500), (1, 210)))  # 设置机械臂初始位置
